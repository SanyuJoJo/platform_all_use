#!/usr/bin/env bash
# ============================================================================
# 构建后端部署包（tar.gz）
#
# v1.3 变更：
#   - 默认不再打包 .venv。原因：venv 内含构建机绝对路径
#     （pyvenv.cfg、.pth、lib64 符号链接等），解压到目标机后会加载
#     错误的 Python 库。改为由 install.sh 在目标机重建 .venv。
#   - 提供 INCLUDE_VENV=1 环境变量以启用旧的打包 .venv 行为（仅当
#     构建环境与运行环境为同一台机器时使用）。
#
# v1.2 新增：
#   - --flat：生成无顶层目录的 backend-flat-*.tar.gz，
#            可直接用于 create_package/bin/bin/backend.tar.gz。
#
# v1.1 修复：
#   - P1-3：兼容 macOS，sha256sum 不存在时回退 shasum -a 256。
#   - P2-3：关键文件存在性校验，缺失则报错退出。
#
# 用法：
#   ./scripts/build.sh                # 完整构建（含 ruff 检查）
#   ./scripts/build.sh --skip-lint    # 跳过 ruff 检查
#   ./scripts/build.sh --include-frontend  # 若前端产物存在，一并打包
#   ./scripts/build.sh --flat         # 生成无顶层目录的包（产品部署包推荐）
#   INCLUDE_VENV=1 ./scripts/build.sh --flat  # 打包 .venv（不推荐）
#
# 输出：
#   默认模式：
#     dist/backend-deploy-YYYYMMDD-HHMMSS.tar.gz
#     dist/backend-deploy-YYYYMMDD-HHMMSS.manifest.json
#   Flat 模式（--flat）：
#     dist/backend-flat-YYYYMMDD-HHMMSS.tar.gz
#     dist/backend-flat-YYYYMMDD-HHMMSS.manifest.json
# ============================================================================
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
BACKEND_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
REPO_ROOT="$(cd "$BACKEND_DIR/.." && pwd)"
cd "$BACKEND_DIR"

SKIP_LINT=0
INCLUDE_FRONTEND=0
FLAT=0
INCLUDE_VENV="${INCLUDE_VENV:-0}"

for arg in "$@"; do
    case "$arg" in
        --skip-lint) SKIP_LINT=1 ;;
        --include-frontend) INCLUDE_FRONTEND=1 ;;
        --flat) FLAT=1 ;;
        -h|--help)
            echo "用法：$0 [--skip-lint] [--include-frontend] [--flat]"
            echo ""
            echo "选项："
            echo "  --skip-lint         跳过 ruff 代码检查"
            echo "  --include-frontend  若前端产物存在，一并打包到 frontend/deploy/"
            echo "  --flat              生成无顶层目录的包（backend-flat-*.tar.gz）"
            echo "  -h, --help          显示本帮助"
            echo ""
            echo "环境变量："
            echo "  INCLUDE_VENV=1      打包时包含 .venv（默认不包含，由 install.sh 重建）"
            exit 0
            ;;
        *)
            echo "❌ 未知参数：$arg"
            echo "   可用参数：--skip-lint / --include-frontend / --flat / -h / --help"
            exit 1
            ;;
    esac
done

# ---------- 准备 ----------
DIST_DIR="$BACKEND_DIR/dist"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)

if [ "$FLAT" -eq 1 ]; then
    PACKAGE_NAME="backend-flat-${TIMESTAMP}"
else
    PACKAGE_NAME="backend-deploy-${TIMESTAMP}"
fi

STAGING_DIR="$DIST_DIR/$PACKAGE_NAME"
ARCHIVE_PATH="$DIST_DIR/${PACKAGE_NAME}.tar.gz"
MANIFEST_PATH="$DIST_DIR/${PACKAGE_NAME}.manifest.json"

rm -rf "$STAGING_DIR"
mkdir -p "$STAGING_DIR" "$DIST_DIR"

# ---------- 关键文件存在性校验 ----------
echo "🔍 校验关键文件..."
REQUIRED_ITEMS=("src" "migrations" "scripts" "alembic.ini" "pyproject.toml" "uv.lock")
MISSING=()
for item in "${REQUIRED_ITEMS[@]}"; do
    if [ ! -e "$BACKEND_DIR/$item" ]; then
        MISSING+=("$item")
    fi
done
if [ "${#MISSING[@]}" -gt 0 ]; then
    echo "❌ 缺少关键文件/目录：${MISSING[*]}"
    exit 1
fi
echo "✅ 关键文件齐全"

# ---------- 代码风格检查 ----------
if [ "$SKIP_LINT" -eq 0 ] && command -v ruff >/dev/null 2>&1; then
    echo "🔍 执行 ruff check..."
    if ! ruff check src/ tests/; then
        echo ""
        echo "❌ ruff check 失败。建议修复步骤："
        echo "   1. 自动修复：ruff check --fix src/ tests/"
        echo "   2. 查看剩余：ruff check src/ tests/"
        echo "   3. 临时跳过：$0 --skip-lint"
        exit 1
    fi
    echo "✅ ruff check 通过"
elif [ "$SKIP_LINT" -eq 1 ]; then
    echo "⏭  已跳过 ruff check（--skip-lint）"
fi

# ---------- 复制文件 ----------
echo "📦 复制后端文件..."
copy_item() {
    local src="$1"
    local dst="$2"
    if [ -e "$src" ]; then
        mkdir -p "$(dirname "$dst")"
        cp -R "$src" "$dst"
    fi
}

copy_item "$BACKEND_DIR/src"                    "$STAGING_DIR/src"
copy_item "$BACKEND_DIR/migrations"             "$STAGING_DIR/migrations"
copy_item "$BACKEND_DIR/scripts"                "$STAGING_DIR/scripts"
copy_item "$BACKEND_DIR/alembic.ini"            "$STAGING_DIR/alembic.ini"
copy_item "$BACKEND_DIR/pyproject.toml"         "$STAGING_DIR/pyproject.toml"
copy_item "$BACKEND_DIR/uv.lock"                "$STAGING_DIR/uv.lock"
copy_item "$BACKEND_DIR/.env.production.example" "$STAGING_DIR/.env.production.example"
copy_item "$BACKEND_DIR/Dockerfile"             "$STAGING_DIR/Dockerfile"
copy_item "$BACKEND_DIR/.dockerignore"          "$STAGING_DIR/.dockerignore"

# ---------- 是否包含 .venv ----------
if [ "$INCLUDE_VENV" -eq 1 ]; then
    if [ -d "$BACKEND_DIR/.venv" ]; then
        echo "📦 复制虚拟环境 .venv（INCLUDE_VENV=1）..."
        copy_item "$BACKEND_DIR/.venv" "$STAGING_DIR/.venv"
    else
        echo "⚠️  未找到 $BACKEND_DIR/.venv，跳过"
    fi
else
    echo "⏭  跳过 .venv（由 install.sh 在目标机重建，避免构建机绝对路径问题）"
fi

# 移除 __pycache__
find "$STAGING_DIR" -type d -name "__pycache__" -prune -exec rm -rf {} + 2>/dev/null || true
find "$STAGING_DIR" -type f -name "*.pyc" -delete 2>/dev/null || true

# ---------- 可选：包含前端产物 ----------
if [ "$INCLUDE_FRONTEND" -eq 1 ]; then
    if [ -d "$REPO_ROOT/frontend/deploy/main-app" ]; then
        echo "📦 包含前端产物..."
        mkdir -p "$STAGING_DIR/frontend"
        cp -R "$REPO_ROOT/frontend/deploy" "$STAGING_DIR/frontend/deploy"
    else
        echo "⚠️  未找到前端产物（frontend/deploy/main-app），跳过"
    fi
fi

# ---------- 生成部署说明 ----------
cat > "$STAGING_DIR/DEPLOY.txt" <<'EOF'
============================================================
后端部署包说明
============================================================
1. 解压
   默认模式（含顶层目录）：
     tar -xzf backend-deploy-*.tar.gz
     cd backend-deploy-*
   Flat 模式（--flat，无顶层目录）：
     tar -xzf backend-flat-*.tar.gz
     # 直接在当前目录得到 src/、migrations/ 等

2. 准备环境
   - 安装 Python 3.12+ 与 uv（https://astral.sh/uv）
     或使用 python3 自带 venv
   - 复制 .env.production.example 为 .env 并修改关键配置：
       cp .env.production.example .env
       # 必须修改 SECRET_KEY / LICENSE_SECRET_KEY
       # 建议修改 CORS_ORIGINS

3. 安装依赖并创建虚拟环境
   # 推荐（若已安装 uv）：
   uv venv --python 3.12
   uv sync --frozen --no-dev      # 若有 uv.lock
   # 或
   uv pip install -e .            # 无 uv.lock 时

   # 或使用标准库 venv：
   python3 -m venv .venv
   .venv/bin/pip install --upgrade pip
   .venv/bin/pip install -e .

4. 数据库迁移
   .venv/bin/alembic upgrade head

5. 启动服务
   .venv/bin/uvicorn src.main:app --host 0.0.0.0 --port 8000
   # 或
   ./scripts/start.sh

6. 停止服务
   ./scripts/stop.sh

7. 健康检查
   curl http://localhost:8000/health

8. 容器化部署（可选）
   docker build -t platform-backend .
   docker run -d -p 8000:8000 --env-file .env \
     -v $(pwd)/data:/app/data \
     -v $(pwd)/logs:/app/logs \
     -v $(pwd)/uploads:/app/uploads \
     -v <前端产物路径>:/app/frontend/deploy:ro \
     platform-backend

说明：
  - 本部署包默认不含 .venv，需要在目标机重建（步骤 3）。
    这是因为 venv 内含构建机绝对路径，跨机器不可重定位。
  - 若构建机与目标机为同一台，可用 INCLUDE_VENV=1 重新构建，
    此时可跳过步骤 3，直接执行步骤 4。
============================================================
EOF

# ---------- 打包 ----------
if [ "$FLAT" -eq 1 ]; then
    echo "📦 打包 tar.gz（--flat：无顶层目录）..."
    tar -czf "$ARCHIVE_PATH" -C "$STAGING_DIR" .
else
    echo "📦 打包 tar.gz（含顶层目录）..."
    tar -czf "$ARCHIVE_PATH" -C "$DIST_DIR" "$PACKAGE_NAME"
fi

# ---------- SHA256 ----------
echo "📄 计算 SHA256..."
if command -v sha256sum >/dev/null 2>&1; then
    SHA256=$(sha256sum "$ARCHIVE_PATH" | awk '{print $1}')
elif command -v shasum >/dev/null 2>&1; then
    SHA256=$(shasum -a 256 "$ARCHIVE_PATH" | awk '{print $1}')
else
    echo "⚠️  未找到 sha256sum 或 shasum，跳过 SHA256 计算"
    SHA256="unavailable"
fi

SIZE=$(stat -c%s "$ARCHIVE_PATH" 2>/dev/null || stat -f%z "$ARCHIVE_PATH")

# ---------- 生成 manifest ----------
cat > "$MANIFEST_PATH" <<EOF
{
  "package": "${PACKAGE_NAME}.tar.gz",
  "created_at": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "size_bytes": ${SIZE},
  "sha256": "${SHA256}",
  "include_frontend": $([ "$INCLUDE_FRONTEND" -eq 1 ] && echo "true" || echo "false"),
  "include_venv": $([ "$INCLUDE_VENV" -eq 1 ] && echo "true" || echo "false"),
  "flat": $([ "$FLAT" -eq 1 ] && echo "true" || echo "false")
}
EOF

# ---------- 清理 staging ----------
rm -rf "$STAGING_DIR"

# ---------- 输出 ----------
echo ""
echo "✅ 构建完成"
echo "   部署包：$ARCHIVE_PATH"
echo "   Manifest：$MANIFEST_PATH"
echo "   SHA256：$SHA256"
echo "   大小：$(numfmt --to=iec "$SIZE" 2>/dev/null || echo "${SIZE} bytes")"
echo "   模式：$([ "$FLAT" -eq 1 ] && echo "flat（无顶层目录）" || echo "默认（含顶层目录）")"
echo "   包含 .venv：$([ "$INCLUDE_VENV" -eq 1 ] && echo "是" || echo "否（推荐，由 install.sh 重建）")"

if [ "$FLAT" -eq 1 ]; then
    echo ""
    echo "📌 提示：可将 Flat 包复制到产品部署包目录："
    echo "   cp $ARCHIVE_PATH $REPO_ROOT/create_package/bin/bin/backend.tar.gz"
fi
