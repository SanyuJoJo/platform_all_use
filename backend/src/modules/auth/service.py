...
v1.4 修复：
- 将历史两段格式权限 `dashboard:view` 统一为三段格式
  `platform:dashboard:view`，符合《模块开发规范》§ 六。
  同步更新 DEFAULT_PERMISSIONS 与 GUEST_PERMISSION_CODES。
"""
...
DEFAULT_PERMISSIONS: List[Dict[str, str]] = [
    # 平台核心
    # ★ v1.4 修复：dashboard:view → platform:dashboard:view（三段格式）
    {
...
# ★ v1.4 修复：dashboard:view → platform:dashboard:view
